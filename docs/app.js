// GitHub Pages (padp.github.io/die-history) has no backend of its own -
// this talks to the Render-hosted API across origins, same pattern as
// Granco Saw Monitor's and picos' docs/app.js. Everything else here is
// identical to the local web/app.js, which talks to its own same-origin
// Flask app instead (relative /api/... paths).
const API_BASE = "https://die-history.onrender.com";
...
async function loadSummary() {
  ...
  if (!s.n_dies) {
    totalsEl.textContent = "no data yet";
    return;
  }
...
async function loadList() {
  ...
  const r = await fetch(`${API_BASE}/api/dies?${params}`);
...
async function selectDie(dieNo) {
  ...
  const r = await fetch(`${API_BASE}/api/die/${encodeURIComponent(dieNo)}`);
  if (!r.ok) {
